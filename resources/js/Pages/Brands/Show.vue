  ...
  <AppLayout>
    <div>
      ...
      <div class="flex items-center gap-5 mb-8 p-6 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
        ...
        <div v-else class="w-20 h-20 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-2xl flex items-center justify-center text-3xl font-extrabold text-white shrink-0">
          {{ brand.name[0] }}
        </div>
        <div>
          <nav class="text-sm text-gray-500 mb-1 flex items-center gap-2">
            ...
            <Link :href="route('gadgets.index')" class="hover:text-violet-600 dark:hover:text-violet-400">Products</Link> /
          ...
          <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 sticky top-24 shadow-sm">
            ...
            <div class="space-y-1">
              <Link v-for="opt in sortOptions" :key="opt.value"
                    ...
                    :class="filters.sort === opt.value ? 'bg-violet-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'">
                {{ opt.label }}
              </Link>
            </div>
          </div>
