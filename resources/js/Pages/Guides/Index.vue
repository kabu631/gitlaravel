<template>
  <AppLayout>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">📖 Tech Guides</h1>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <Link v-for="guide in guides.data" :key="guide.id" :href="route('guides.show', guide.slug)"
        ...
        <div class="aspect-video overflow-hidden bg-gray-100 dark:bg-gray-800">
          <img v-if="guide.thumbnail" :src="`/storage/${guide.thumbnail}`" class="w-full h-full object-cover group-hover:scale-105 transition duration-300"/>
          <div v-else class="w-full h-full flex items-center justify-center text-5xl">📖</div>
        </div>
        <div class="p-5">
          <h2 class="font-bold line-clamp-2 text-gray-900 dark:text-gray-100 group-hover:text-violet-600 dark:group-hover:text-violet-300 transition">{{ guide.title }}</h2>
          <p class="text-sm text-gray-500 mt-2">{{ new Date(guide.created_at).toLocaleDateString('en-NP') }}</p>
        </div>
      </Link>
    </div>
    <div v-if="!guides.data.length" class="text-center py-20 text-gray-500">
      <p class="text-5xl mb-4">📖</p><p class="text-xl font-semibold">No guides found</p>
    </div>
    <div v-if="guides.last_page > 1" class="flex justify-center gap-2 mt-10">
