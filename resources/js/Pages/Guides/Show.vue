  ...
  <AppLayout>
    <div class="max-w-3xl mx-auto">
      <nav class="text-sm text-gray-500 mb-6 flex items-center gap-2">
        ...
        <Link :href="route('guides.index')" class="hover:text-violet-600 dark:hover:text-violet-400">Guides</Link> /
      ...
      <img v-if="guide.thumbnail" :src="`/storage/${guide.thumbnail}`" class="w-full rounded-2xl mb-8 max-h-80 object-cover"/>
      <div class="prose prose-gray prose-violet dark:prose-invert max-w-none" v-html="guide.content"/>
      <div v-if="related.length" class="mt-12 pt-8 border-t border-gray-200 dark:border-gray-800">
        ...
        <div class="grid sm:grid-cols-3 gap-4">
          <Link v-for="r in related" :key="r.id" :href="route('guides.show', r.slug)"
                class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-violet-500 dark:hover:border-violet-600 rounded-xl overflow-hidden group transition shadow-sm">
            <img v-if="r.thumbnail" :src="`/storage/${r.thumbnail}`" class="w-full h-28 object-cover"/>
            <div class="p-3">
              <p class="text-sm font-semibold text-gray-800 dark:text-gray-200 line-clamp-2 group-hover:text-violet-600 dark:group-hover:text-violet-300 transition">{{ r.title }}</p>
            </div>
          </Link>
        </div>
      </div>
