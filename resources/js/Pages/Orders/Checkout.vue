            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-3">
                <label v-for="m in methods" :key="m.val" class="flex flex-col items-center gap-2 p-4 bg-white dark:bg-gray-900 border-2 rounded-xl cursor-pointer transition"
                       :class="form.payment_method === m.val ? 'border-brand-500 bg-brand-50 dark:bg-navy-900/20' : 'border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-600'">
        ...
        <div class="lg:col-span-2">
          <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 sticky top-24">
            ...
            <div v-for="item in items" :key="item.id" class="flex gap-3 mb-3 pb-3 border-b border-gray-200 dark:border-gray-800 last:border-0">
              ...
              <p class="text-sm font-bold text-brand-500 dark:text-brand-400 shrink-0">NPR {{ formatPrice(item.quantity * (item.unit_price ?? item.gadget.price)) }}</p>
            </div>
...
<style scoped>
.input { @apply w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-800 dark:text-gray-200 outline-none focus:border-brand-500 transition; }
</style>
