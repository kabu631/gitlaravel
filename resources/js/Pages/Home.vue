<template>
  <AppLayout>
    ...
    <section v-else class="relative rounded-2xl overflow-hidden bg-gradient-to-br from-navy-900 via-navy-800 to-navy-700 dark:from-navy-950 dark:via-gray-900 dark:to-gray-950 px-6 py-12 mb-8 text-center">
      <div class="absolute inset-0 opacity-20" style="background:radial-gradient(circle at 50% 50%,#263248 0%,transparent 70%)"></div>
      <div class="relative">
        <h1 class="text-3xl md:text-5xl font-extrabold mb-3 bg-gradient-to-r from-brand-200 to-amber-100 bg-clip-text text-transparent">
          Nepal's #1 Tech Price Tracker
        </h1>
        <p class="text-brand-100 dark:text-gray-400 mb-6 max-w-lg mx-auto">Compare prices, read expert reviews, and find the best tech deals in Nepal.</p>
        <div class="flex gap-3 justify-center flex-wrap text-sm">
          <Link :href="route('gadgets.index')" class="px-5 py-2.5 bg-white text-navy-800 hover:bg-brand-50 rounded-xl font-semibold transition shadow-sm">Browse Products</Link>
          <Link :href="route('compare.index')" class="px-5 py-2.5 bg-navy-700/60 hover:bg-navy-700/80 text-white rounded-xl font-semibold transition">Compare Gadgets</Link>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        <section v-if="news.length">
          ...
          <div class="grid sm:grid-cols-3 gap-4">
            <Link v-for="(article, i) in news.slice(0, 3)" :key="article.id"
              ...
              <span class="absolute top-3 left-3 z-10 w-7 h-7 bg-brand-500 rounded-full text-xs font-bold flex items-center justify-center text-white">{{ i + 1 }}</span>
              ...
              <div class="p-3">
                <span class="text-xs text-brand-500 dark:text-brand-400 font-bold uppercase tracking-wide">{{ article.category }}</span>
                <h3 class="text-sm font-semibold mt-1 line-clamp-2 group-hover:text-brand-500 dark:group-hover:text-brand-300 transition">{{ article.title }}</h3>
              </div>
            </Link>
          </div>
        </section>
        ...
        <section v-if="featured.length">
          ...
          <div class="flex gap-2 mb-4 overflow-x-auto pb-1 scrollbar-hide">
            <button @click="activeTab = 'all'"
                    :class="activeTab === 'all' ? 'bg-brand-500 text-white border-brand-500' : 'bg-transparent text-gray-500 dark:text-gray-400 border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'"
            ...
            <button v-for="cat in featuredCategories" :key="cat.slug"
                    ...
                    :class="activeTab === cat.slug ? 'bg-brand-500 text-white border-brand-500' : 'bg-transparent text-gray-500 dark:text-gray-400 border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500'"
        ...
        <section v-if="reviews.length">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <Link v-for="review in reviews" :key="review.id"
              ...
              <span class="absolute top-3 right-3 text-xs bg-brand-100 dark:bg-brand-500/20 text-brand-500 dark:text-brand-400 border border-brand-300 dark:border-brand-700/50 rounded-full px-2 py-0.5 font-semibold">
                ✦ EDITOR'S PICK
              </span>
              ...
              <div class="min-w-0 pr-16">
                ...
                <h3 class="font-bold text-sm group-hover:text-brand-500 dark:group-hover:text-brand-300 transition line-clamp-2 mt-0.5">{{ review.title }}</h3>
        ...
        <!-- Compare CTA -->
        <section>
          <div class="rounded-2xl p-6 text-center border border-brand-200 dark:border-navy-700/50"
               style="background:linear-gradient(135deg,rgba(38,50,72,.08) 0%,rgba(26,34,52,.05) 100%)">
            ...
            <Link :href="route('compare.index')"
                  class="inline-block px-6 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-400 text-white font-bold text-sm transition">
              Start Comparing →
            </Link>
          </div>
        </section>
      ...
      <aside class="space-y-5">
          ...
          <div class="space-y-3 mt-3">
            <Link v-for="(g, i) in trending.slice(0, 6)" :key="g.id"
              ...
              <span class="w-6 h-6 shrink-0 rounded-full text-xs font-bold flex items-center justify-center"
                    :class="i < 3 ? 'bg-brand-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'">
                {{ i + 1 }}
              </span>
              ...
              <div class="min-w-0 flex-1">
                <p class="text-xs font-semibold line-clamp-1 group-hover:text-brand-500 dark:group-hover:text-brand-300 transition">{{ g.name }}</p>
                <p class="text-xs text-brand-500 dark:text-brand-400 font-bold">NPR {{ g.price?.toLocaleString() }}</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          <div class="flex items-center justify-between mb-3">
            ...
            <Link :href="route('pages.price-tracker')" class="text-xs text-brand-500 dark:text-brand-400 hover:text-brand-400 dark:hover:text-brand-300">All Prices →</Link>
          </div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          ...
          <div class="flex flex-wrap gap-2">
            <Link v-for="brand in brands" :key="brand.id"
                  ...
                  class="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-brand-500 dark:hover:bg-brand-600 border border-gray-200 dark:border-gray-700 hover:border-brand-400 dark:hover:border-brand-500 rounded-full text-xs font-medium text-gray-700 dark:text-gray-300 hover:text-white transition">
              {{ brand.name }}
            </Link>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
          ...
          <div class="space-y-1">
            <Link v-for="cat in categories" :key="cat.id"
              ...
              <span class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 group-hover:text-brand-500 dark:group-hover:text-brand-300">
...
.view-all       { @apply text-brand-500 dark:text-brand-400 hover:text-brand-400 dark:hover:text-brand-300 text-sm transition; }
