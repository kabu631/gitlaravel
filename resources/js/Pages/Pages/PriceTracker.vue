<template>
  <AppLayout>
    <!-- Header -->
    <div class="bg-brand-500 dark:bg-navy-900 border-b border-navy-600 dark:border-navy-900">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-center">
        ...
        <p class="text-brand-200 text-lg max-w-2xl mx-auto">{{ subheading }}</p>
      </div>
    </div>
      ...
      <div class="grid lg:grid-cols-4 gap-8 items-start">
            ...
            <div class="space-y-4">
              <Link v-for="gadget in trending" :key="gadget.id" :href="route('gadgets.show', gadget.slug)" class="flex gap-3 group">
                ...
                <div>
                  ...
                  <p class="text-xs font-bold text-brand-500 mt-0.5">NPR {{ gadget.price?.toLocaleString() }}</p>
                </div>
              </Link>
            </div>
          </div>
          ...
          <div v-if="gadgets.data.length" class="space-y-6">
            <div v-for="gadget in gadgets.data" :key="gadget.id" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm">
              <div class="flex items-start gap-4 mb-4 border-b border-gray-100 dark:border-gray-800 pb-4">
                ...
                <div class="flex-1">
                  <Link :href="route('gadgets.show', gadget.slug)" class="hover:text-brand-500 transition">
                    <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white leading-snug">{{ gadget.brand?.name }} {{ gadget.name }}</h2>
                  </Link>
                  <div class="flex items-center gap-3 mt-2">
                    <span class="text-lg font-extrabold text-brand-500 dark:text-brand-400">NPR {{ gadget.price?.toLocaleString() }}</span>
              ...
              <div v-if="gadget.price_tracker_description" class="prose prose-sm dark:prose-invert max-w-none text-gray-600 dark:text-gray-400">
                <div class="flex items-center gap-2 mb-2">
                  <span class="text-xs font-bold bg-brand-100 dark:bg-navy-900 text-brand-600 dark:text-brand-300 px-2 py-1 rounded uppercase tracking-wider">Editor's Insight</span>
                </div>
        ...
        <div class="lg:col-span-1 space-y-8 sticky top-6">
          <div class="bg-gradient-to-br from-brand-500 to-navy-700 rounded-2xl p-6 text-white text-center shadow-lg">
            ...
            <p class="text-xs text-brand-200 mb-4">Subscribe to get instantly notified when prices drop.</p>
            ...
            <button class="w-full bg-white text-brand-600 font-bold text-sm py-2 rounded-lg hover:bg-gray-50 transition">Notify Me</button>
          </div>
          ...
          <div v-if="sidebarNews?.length" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm">
            ...
            <div class="space-y-4">
              <Link v-for="news in sidebarNews" :key="news.id" :href="route('news.show', news.slug)" class="group block">
                <p class="text-[10px] font-bold text-brand-500 dark:text-brand-400 uppercase tracking-widest mb-1">{{ news.category }}</p>
