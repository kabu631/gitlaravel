  ...
  <StaticPageLayout current-page="about"
    ...
    <div class="max-w-4xl">
      ...
      <div class="text-center mb-12">
        <span class="inline-block bg-violet-600 text-white text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4">About Us</span>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-12">
        <div v-for="stat in stats" :key="stat.label"
             class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 text-center hover:border-violet-400 dark:hover:border-violet-600 transition">
          <p class="text-3xl font-extrabold text-violet-600 dark:text-violet-400">{{ stat.num }}</p>
      ...
      <div class="grid sm:grid-cols-2 gap-4 mb-12">
        <div class="bg-white dark:bg-gray-900 rounded-2xl border-l-4 border-violet-500 p-6">
          <div class="w-12 h-12 bg-violet-100 dark:bg-violet-900/40 rounded-xl flex items-center justify-center text-2xl mb-4">🎯</div>
      ...
      <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-8 mb-12">
        <div class="flex items-start gap-6">
          <div class="w-16 h-16 rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-2xl shrink-0">📖</div>
          <div>
      ...
      <div class="mb-12">
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
          <div v-for="member in team" :key="member.name"
               class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 text-center hover:border-violet-400 dark:hover:border-violet-600 transition">
            <div class="w-20 h-20 rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-3xl mx-auto mb-4">
              👤
            </div>
            <h4 class="font-bold text-lg">{{ member.name }}</h4>
            <p class="text-violet-600 dark:text-violet-400 text-sm font-semibold mb-2">{{ member.role }}</p>
      ...
      <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-10 text-center mb-4">
        ...
        <Link :href="route('pages.contact')"
              class="inline-block px-8 py-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-bold transition">
          Contact Us →
        </Link>
      </div>
    </div>
