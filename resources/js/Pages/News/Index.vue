      ...
      <div class="grid lg:grid-cols-4 gap-8 items-start">
          ...
          <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 shadow-sm">
            ...
            <div class="space-y-1">
              <button v-for="cat in categories" :key="cat.val" @click="filterCategory(cat.val)"
                      ...
                      :class="filters.category === cat.val ? 'bg-brand-500 text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-brand-500'">
                {{ cat.label }}
              </button>
            </div>
          </div>
          ...
          <div v-if="articles.data.length" class="space-y-6">
            <Link v-for="article in articles.data" :key="article.id" :href="route('news.show', article.slug)"
              ...
              <div class="p-5 sm:p-6 flex-1 flex flex-col justify-center">
                <span class="text-xs text-brand-500 dark:text-brand-400 font-extrabold uppercase tracking-widest mb-2">{{ article.category }}</span>
                <h2 class="text-lg sm:text-xl font-bold mb-3 line-clamp-2 text-gray-900 dark:text-gray-100 group-hover:text-brand-500 dark:group-hover:text-brand-400 transition leading-snug">{{ article.title }}</h2>
        ...
        <div class="lg:col-span-1 space-y-8 sticky top-6">
          ...
          <div class="bg-gradient-to-br from-brand-500 to-navy-700 rounded-2xl p-6 text-white text-center shadow-lg">
            ...
            <p class="text-xs text-brand-200 mb-4">Get breaking tech news directly to your inbox.</p>
