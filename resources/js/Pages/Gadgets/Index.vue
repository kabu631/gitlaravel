    ...
    <div class="flex gap-8">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 sticky top-24">
          ...
          <div class="mb-4">
            ...
            <div class="space-y-1">
              <Link :href="route('gadgets.index', { ...filters, category: null })"
                    ...
                    :class="!filters.category ? 'text-brand-500 dark:text-brand-400' : 'text-gray-500 dark:text-gray-400'">All</Link>
              <Link v-for="cat in categories" :key="cat.id"
                    :href="route('gadgets.index', { ...filters, category: cat.slug })"
                    class="block text-sm px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition"
                    :class="filters.category === cat.slug ? 'text-brand-500 dark:text-brand-400' : 'text-gray-500 dark:text-gray-400'">
          ...
          <div class="mb-4">
            ...
            <div class="flex gap-2">
              <input v-model="localFilters.min_price" type="number" placeholder="Min" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-sm text-gray-800 dark:text-gray-200 outline-none focus:border-brand-500"/>
              <input v-model="localFilters.max_price" type="number" placeholder="Max" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-sm text-gray-800 dark:text-gray-200 outline-none focus:border-brand-500"/>
            </div>
            <button @click="applyFilters" class="mt-2 w-full bg-brand-500 hover:bg-brand-500 rounded-lg py-1.5 text-sm font-semibold transition">Apply</button>
          </div>

          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold mb-2 block">Sort By</label>
            <select v-model="localFilters.sort" @change="applyFilters" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200 outline-none focus:border-brand-500">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center justify-between mb-6">
          ...
          <input v-model="localFilters.search" @keyup.enter="applyFilters" type="text" placeholder="Search..." class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm outline-none focus:border-brand-500 w-48"/>
        </div>
